 ...
 * 这里对marked的源代码进行了适当的裁剪和增加，下面是我自己使用的markdown规则
 *
 * 区块元素：
 ...
 *    2. link和image两个元素的title允许存在上下标，并且传入renderer的参数都带有转义之后的标签
 *    3. 除开特别说明了的，特殊字符默认都未被转码，具体来说是这几个字符: & < > " '
    ...
    function inlineLexer(src, render) {
        ...
        while (src) {
            ...
            if (cap = inline.code.exec(src)) {
                src = src.substring(cap[0].length);
                out += render.codespan(inlineLexer(cap[2], render));
                continue;
            }
            // del          删除线
    ...
    function lexer(context) {
        ...
        (function token(txt, top){
            let src = txt.replace(/^ +$/gm, ''),
                next, loose, cap, bull, item, space, i, l;
            while (src) {
                // newline
