...
function createSite() {
    ...
    for (let i = 0; i < posts.length; i++) {
        ...
        for (let j = 0; j < postTags.length; j++) {
            ...
            if (tagsPage[tag]) {
                ...
                tagsPage[tag].total = tagsPage[tag].length;
            } else {
                tagsKey.push(tag);
        ...
        if (catePage[postCate]) {
            ...
            catePage[postCate].total = catePage[postCate].length;
        } else {
        ...
        if (timePage[postDate]) {
            ...
            timePage[postDate].total = timePage[postDate].length;
        } else {
    ...
    //生成主页侧边栏
    site.push({
        path: path.normalize('/index/aside.html'),
    ...
    Object.keys(collect).forEach((name) => {
        ...
        //侧边栏
        site.push({
            path: path.join('/', name, '/aside.html'),
